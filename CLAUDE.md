...
- `_data/usage.yml` — the deletions total, the site's only first-party number. Written **daily** by `.github/workflows/update-usage-total.yml` from `https://tidywl.com/api/stats`; do not hand-edit. `deletedTotal` is a **floor**, rounded down to the nearest 1,000, because every place that renders it says *"over"* — rounding up would make the copy false between runs. `deletedRaw` is the unrounded value the next run's **monotonic guard** compares against: the counters keep MAX-ever-reported and retired rows roll into `retired_total`, so this number cannot legitimately fall, and a drop means a bad response rather than a real decline. Rendered in four places from this one variable — the hero stat, the *"How many videos have TidyWL users actually deleted?"* FAQ answer, that question's `FAQPage` JSON-LD, and `llms.txt` (which carries front matter solely so Liquid renders there). `scripts.html` then hydrates the hero stat live; **the markup figure is what non-JS crawlers index, which is most AI crawlers, so it must never be left hand-maintained** — it went 39% stale in the 24 hours it was.
